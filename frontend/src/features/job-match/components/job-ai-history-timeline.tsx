...
import { useJobAiOutputs } from "../hooks/use-job-tools"
...
/** "Geçmiş İşlemler" — bu ilan+CV çifti için üretilmiş tüm AI çıktıları (ATS
 * Optimize, Ön Yazı, Mülakat Hazırlığı, Maaş Tahmini) paylaşılan TimelineList
 * ile tek bir zaman akışında. CV Detay'daki AiHistoryTimeline ile aynı desen. */
export function JobAiHistoryTimeline({ jobId, cvId }: { jobId: string; cvId: string }) {
        ...
        {outputsQuery.data && outputsQuery.data.length > 0 && (
          <TimelineList
            items={outputsQuery.data.map((output) => ({
              ...
              onClick: () => navigate(getAiOutputUrl(cvId, output.id, jobId)),
            }))}
          />
        )}
      </CardContent>
